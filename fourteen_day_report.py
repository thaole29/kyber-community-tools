"""
fourteen_day_report.py

Generates a support team performance report for the last 14 days (336 hours)
...
def generate_report():
    ...
    cutoff = now - timedelta(hours=336)

    # --- Filter tickets created in the last 14 days ---
    ...
    # --- Average handling time for CLOSED tickets in last 14 days ---
    ...
    report = {
        "title": "📊 14-Day Support Team Report",
        "description": f"Performance summary for the last 14 days\n`{cutoff.strftime('%Y-%m-%d %H:%M')} → {now.strftime('%Y-%m-%d %H:%M')} (UTC+7)`",
        "metrics": {
