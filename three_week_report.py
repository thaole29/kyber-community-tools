"""
three_week_report.py

Generates a support team performance report for the last 3 weeks (21 days / 504 hours)
...
def generate_report():
    ...
    cutoff = now - timedelta(hours=504)

    # --- Filter tickets created in the last 21 days ---
    ...
    # --- Average handling time for CLOSED tickets ---
    ...
    report = {
        "title": "📊 3-Week Support Team Report",
        "description": f"Performance summary for the last 21 days\n`{cutoff.strftime('%Y-%m-%d %H:%M')} → {now.strftime('%Y-%m-%d %H:%M')} (UTC+7)`",
        "metrics": {
